...
const RecentlyViewedPage = () => {
  ...
  useEffect(() => {
    ...
    const syncWishlist = async () => {
      ...
      const favoriteIds = new Set(wishlistData.map(item => item.product.id));
      const map = new Map(wishlistData.map(item => [item.product.id, item.id]));
  ...
  const handleToggleWishlist = async (productId) => {
    ...
    try {
      if (isFavorite) {
        ...
        const newWishlistItem = await addToWishlist(productId);
        if (newWishlistItem) {
            newFavorites.add(productId);
            newWishlistMap.set(productId, newWishlistItem.id);
